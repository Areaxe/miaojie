var areaData = [{ "name": "浙江", "cities": ["杭州", "金华", "宁波", "温州", "嘉兴", "绍兴", "丽水", "湖州", "台州", "舟山", "衢州"] },
...
function initProvince() {
    ...
    province.onchange = function () {
        initCity(areaData[this.selectedIndex].cities)
    }
    initCity(areaData[0].cities)
}
function initCity(cityList) {
...
function animateFunc() {
    var storeImgs = document.querySelectorAll('.openStore-right img')
    Array.prototype.map.call(storeImgs, function (v, i) {
        v.onclick = function () {
            changeStoreImg(v.getAttribute('class'))
        }
    })
    console.log(storeImgs)
}
function changeStoreImg(currentImg) {
    var left = document.querySelector('.openStore-right .left')
    var center = document.querySelector('.openStore-right .center')
    var right = document.querySelector('.openStore-right .right')
    switch (currentImg) {
        case 'left':
            left.setAttribute('class', 'center')
            center.setAttribute('class', 'right')
